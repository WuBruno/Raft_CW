...
defmodule Log do
  ...
  def append_entry(s, entry), do: Log.new(s, Map.put(s.log, Log.last_index(s) + 1, entry))

  # entries should be disjoint
  def merge_entries(s, entries), do: Log.new(s, Map.merge(s.log, entries))

  # e.g. delete s.log[3..5] keep rest
  def delete_entries(s, range), do: Log.new(s, Map.drop(s.log, Enum.to_list(range)))

  # delete s.log[from..last] keep rest
  def delete_entries_from(s, from), do: Log.delete_entries(s, from..Log.last_index(s))
end
